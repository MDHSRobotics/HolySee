#include "HighgoalAR.h"



HighgoalAR::HighgoalAR() : Filter(name)
{
}
...
void HighgoalAR::process(cv::Mat& source0){
	//Step HSV_Threshold0:
	//input
...
void HighgoalAR::makeMatFromContours(cv::Mat & original,std::vector<std::vector<cv::Point> > & contours,cv::Mat &arImage){
//loop through contours
	...
	//loop through points
		for(int p=0;p<contour.size();p++){
			...
			drawContours(arImage, contours, -1, (0,255,0), 3);
		}
...
void HighgoalAR::setsource(cv::Mat &source0){
 ...
 */
cv::Mat* HighgoalAR::gethsvThresholdOutput(){
...
std::vector<std::vector<cv::Point> >* HighgoalAR::getfindContoursOutput(){
 ...
 */
std::vector<std::vector<cv::Point> >* HighgoalAR::getfilterContoursOutput(){
	 ...
	 */
	void HighgoalAR::hsvThreshold(cv::Mat &input, double hue[], double sat[], double val[], cv::Mat &out) {
	 ...
	 */
	void HighgoalAR::findContours(cv::Mat &input, bool externalOnly, std::vector<std::vector<cv::Point> > &contours) {
	 ...
	 */
	void HighgoalAR::filterContours(std::vector<std::vector<cv::Point> > &inputContours, double minArea, double minPerimeter, double minWidth, double maxWidth, double minHeight, double maxHeight, double solidity[], double maxVertexCount, double minVertexCount, double minRatio, double maxRatio, std::vector<std::vector<cv::Point> > &filterContoursOutput) {
